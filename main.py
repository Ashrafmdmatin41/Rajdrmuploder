...
# Config 
class Config(object):
    BOT_TOKEN = os.environ.get("BOT_TOKEN", "6485798868:AAFio1FOHbTwXzZK4ZznwsODNHZOv3KwcVI")
    API_ID = int(os.environ.get("API_ID",  27923601))
    API_HASH = os.environ.get("API_HASH", "45f49c6a237634652634858b393fb11d")
    DOWNLOAD_LOCATION = "./DOWNLOADS"
    SESSIONS = "./SESSIONS"

    AUTH_USERS = os.environ.get('AUTH_USERS', '5970997865').split(',')
    for i in range(len(AUTH_USERS)):
        AUTH_USERS[i] = int(AUTH_USERS[i])

    GROUPS = os.environ.get('GROUPS', '-1001817885963').split(',')
    for i in range(len(GROUPS)):
        GROUPS[i] = int(GROUPS[i])

    LOG_CH = -1001997443664
